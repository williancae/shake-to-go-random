...
export default function SpinWheel({ products, onSpin }: SpinWheelProps) {
  ...
  // NOVO getIndex: inverte o ângulo para coincidir com a seta apontando para cima
  const getIndex = useCallback(() => {
    if (products.length === 0) return 0;

    const sectorAngle = TAU / products.length;
    // Normaliza e inverte o ângulo
  ...
  const handleSpin = () => {
    ...
    const randomness = (Math.random() - 0.5) * arc * 0.5;
    const target = mid + randomness;
    const rotations = 2 + Math.random() * 2;
    const final = target + rotations * TAU;
    const diff = final - (ang % TAU);
    const initVel = diff * (1 - friction);

    setAngVel(Math.abs(initVel));
